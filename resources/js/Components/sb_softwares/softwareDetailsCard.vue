...
const softwareId = props.details.id;
const csrfToken = ref('');
const total_price = ref(0);
const business_categories_id = ref('');

const isActive = ref(false);
const WBActive = ref(false);
...
const inputWHValue = computed(() => {
  return WBActive.value ? 1 : 0;
});

const selectedBCateg = computed(() => {
    if (!formData.value.business_categories_id) {
      total_price.value=0;
      return null
    }
  //  return props.AllBusinessCategories.find(BCateg => BCateg.id === formData.value.business_categories_id)
    return props.AllBusinessCategories.find(BCateg => BCateg.id === formData.value.business_categories_id)
  })
 
   async function getPrice()  {
  try {
      total_price.value=props.AllBusinessCategories.find(BCateg => BCateg.id === formData.value.business_categories_id).price;



    // if (inputValue.value) {
    // //  total_price.value = total_price.value+50;
    // }

    if (inputWHValue.value) {
        total_price.value = total_price.value+1200;
      }
...
const nextDueDate = new Date(today.getTime() + (30 * 24 * 60 * 60 * 1000));
const nextDueDate2 = nextDueDate.toISOString().slice(0, 10);
console.log(nextDueDate2)
const data=ref({});
const formData = ref({
...
const date = new Date();
console.log(formData.value.order_date);

data.value.order_date=date.toISOString().slice(0, 10);
data.value.automatically_renew=inputValue.value;
data.value.price=total_price.value;
data.value.number_of_device=selectedBCateg.value.number_of_devices;
data.value.total_before_tax=total_price.value;
data.value.total_after_tax=total_price.value;
data.value.next_due_date=formData.value.next_due_date;
data.value.web_hosting=inputWHValue.value;
data.value.demon_version=formData.value.demon_version;
data.value.business_categories_id=formData.value.business_categories_id;
data.value.user_id=formData.value.user_id;
data.value.software_id=softwareId;
data.value.order_status_id=formData.value.order_status_id;
data.value.server_id=formData.value.server_id;
  ...
  //  const response = await axios.post('/orders', data.value);
    // console.log(response.data);
    // console.log(response.data.id);
    // console.log(response.data.order_date);
    // console.log(response.data.automatically_renew);
    // console.log(response.data.price);
    // console.log(response.data.number_of_device);
    // console.log(response.data.total_before_tax);
    // console.log(response.data.total_after_tax);
    // console.log(response.data.next_due_date);
    // console.log(response.data.web_hosting);
    // console.log(response.data.demon_version);
    // console.log(response.data.business_categories_id);
    // console.log(response.data.user_id);
    // console.log(response.data.software_id);
    // console.log(response.data.order_status_id);
    // console.log(response.data.server_id);
    // console.log(response.data.created_at);
    // console.log(response.data.updated_at);
    // console.log(response.data.deleted_at);
    // console.log(response.data.user);

console.log("data ",data)
    // Create form data object to send file data

//     const data = new FormData();
//     data.append('order_date', formData.value.order_date);
//     // data.append('automatically_renew', formData.value.automatically_renew);
//     data.append('automatically_renew', inputValue.value);

//     data.append('price', selectedBCateg.value.price );
//     data.append('number_of_device', selectedBCateg.value.number_of_devices);
//     data.append('total_before_tax',  selectedBCateg.value.price);
//     data.append('total_after_tax',  selectedBCateg.value.price);
//     data.append('next_due_date', formData.value.next_due_date);
//     // data.append('web_hosting', formData.value.web_hosting);
//     data.append('web_hosting', inputWHValue.value);
      ...
      <form @submit.prevent="submitForm">
        ...
        <div class="py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div class="sm:col-span-3">
            ...
            <div class="mt-2">
              <select v-model="formData.business_categories_id" 
              ...
              @change="handleOptionChange"
                ...
                <option v-for="BCateg in AllBusinessCategories" :value="BCateg.id" :key="BCateg.id">{{ BCateg.name }}
                </option>
              </select>
            </div>
        </div>
          <div class="sm:col-span-3">
            <label for="trial" class="block text-sm font-medium leading-6 text-gray-900">
              Max Devices </label>
            <div class="mt-2">
              <select v-model="formData.business_categories_id" @change="handleOptionChange" name="business_categories_id"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                required>

                <option v-for="BCateg in AllBusinessCategories" :value="BCateg.id" :key="BCateg.id">{{
                  BCateg.number_of_devices }}
        ...
        <div class="py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div class="flex items-center justify-center">
            ...
            <label for="toggleThree" class="flex cursor-pointer select-none items-center">
              <div class="relative">
                <input type="checkbox" id="toggleThree" 
                class="sr-only" 
                v-model="isActive"
                @change="handleOptionChange"
                />
        ...
        <div class="py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div class="flex items-center justify-center">
            ...
            <label for="WBToggleThree" class="flex cursor-pointer select-none items-center">
              <div class="relative">
                <input type="checkbox" 
                id="WBToggleThree" class="sr-only" 
                v-model="WBActive"
                @change="handleOptionChange"
                />
                <div class="block h-8 w-14 rounded-full bg-[#E5E7EB]"></div>
        ...
        <div class="flex justify-between my-9 items-center w-full">
          ...
          <div class="sm:col-span-3">
            <span v-if="selectedBCateg" >
              {{ total_price }},00DH</span>

            <div class="mt-2">
              <!-- {{ getPrice(formData.business_categories_id) }} -->
        ...
        <div>
          <div class="border-t border-b py-4 mt-7 border-gray-200">
            <div data-menu class="flex justify-between items-center cursor-pointer">
              <p class="text-base leading-4 text-gray-800 dark:text-gray-300">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="60" height="40" version="1.1"
                  ...
                  image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 360 511.48"
            ...
            <div class="hidden pt-4 text-base leading-normal pr-12 mt-4 text-gray-600 dark:text-gray-300" id="sect">You
              will
  ...
  <div v-if="showModalCreate"
    ...
    <div class=" modal-content relative w-screen px-20 max-h-full">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <button @click="showModalCreate = false" type="button"
        ...
        <paymentForm :price="selectedBCateg.price" />
        <div class="  flex items-center justify-end gap-x-6 mr-40 ">
          <button @click="showModalCreate = false" type="button" class="text-sm font-semibold leading-6 text-gray-900">
            <span class="sr-only">Close modal</span>
            Cancel</button>
        </div>
      </div>
